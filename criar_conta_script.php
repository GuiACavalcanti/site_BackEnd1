...
if ($nome_txt == null || $email_txt == null || $email_txt == null) {
    echo "<script>alert('Erro: O formulario deve ser preenchido'); location.href = 'criar_conta.html';</script>";
} else if ($email_txt == $User_login_email['email'] && $nome_txt == $User_login_email['nome']) {
    echo "<script>
    alert('Erro: Usuario já existe!');
    location.href = 'criar_conta.html';
    </script>";
} else if ($con->query("INSERT INTO site_1 (nome, email, senha) VALUE ('$nome_txt', '$email_txt', '$senha_txt');") === true) {
        echo "
        <script>
